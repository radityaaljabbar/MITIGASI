...
const DetailNilaiContent = ({ nim }) => {
    ...
    // Function to get tingkat based on semester
    const getTingkatFromSemester = (semester) => {
        ...
        return 0; // For unknown semesters
    };

    // Filter courses based on selected tingkat
    useEffect(() => {
    ...
    // Function to get row styling based on grade
    const getRowStyle = (grade) => {
        if (!grade) return '';

        const cleanGrade = grade.trim().toUpperCase();

        // Define grade ranges with colors
        const gradeColors = {
            A: 'bg-green-50 border-l-4 border-green-500',
            AB: 'bg-green-50 border-l-4 border-green-400',
            B: 'bg-blue-50 border-l-4 border-blue-500',
            BC: 'bg-blue-50 border-l-4 border-blue-400',
            C: 'bg-yellow-50 border-l-4 border-yellow-500',
            D: 'bg-orange-50 border-l-4 border-orange-500',
            E: 'bg-red-50 border-l-4 border-red-500',
            F: 'bg-red-100 border-l-4 border-red-600',
        };

        return gradeColors[cleanGrade] || '';
    };

    // Function to get grade badge styling
    const getGradeBadge = (grade) => {
        ...
        const badgeColors = {
            ...
            AB: 'bg-green-100 text-green-700 font-semibold',
            B: 'bg-blue-100 text-blue-800 font-semibold',
            BC: 'bg-blue-100 text-blue-700 font-semibold',
    ...
    // Function to get tingkat badge styling
                ...
                <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-center px-6 pt-6 gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                        ...
                        <div className="text-sm text-gray-600 bg-gray-100 px-3 py-2 rounded-full">
                            ...
                            {courseHistory.length} mata kuliah
                        </div>
                    </div>
                </div>
                        ...
                        <thead className="sticky top-0 z-10">
                            <tr className="bg-[#951a22] text-white">
                                {[
                                    ...
                                    <th
                                        key={index}
                                        className="border border-gray-300 p-3 text-center font-bold uppercase bg-[#951a22] first:rounded-tl-lg last:rounded-tr-lg">
                                        {header}
                                    </th>
                                ))}
                            </tr>
                                    ...
                                    <tr
                                        ...
                                        <td className="p-3 text-center border-b border-gray-200">
                                            <div className="flex flex-col items-center gap-1">
                                                <span
                                                    className={`px-2 py-1 rounded text-xs font-medium ${getTingkatBadge(
                                                        course.semester
                                                    )}`}>
                                                    Semester{' '}
                                                    {course.semester || '-'}
                                        ...
                                        <td className="p-3 text-center border-b border-gray-200">
                                            <span
                                                className={`px-3 py-1 rounded-full text-sm font-bold ${getGradeBadge(
                                                    course.nilai
                                                )}`}>
                                                {course.nilai
                                                    ? course.nilai.trim()
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-lg font-medium">
                                                {selectedTingkat === 'semua'
                                                    ...
                                                    : `Tidak ada mata kuliah pada Tingkat ${selectedTingkat}`}
                                            </p>
                                            <p className="text-sm text-gray-400">
                                                {selectedTingkat === 'semua'
                                                    ? 'Data akan muncul setelah mahasiswa mengambil mata kuliah'
                                                    : 'Coba pilih tingkat lain untuk melihat data mata kuliah'}
                                            </p>
                                        </div>
                                    </td>
                                </tr>
